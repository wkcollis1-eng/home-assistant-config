...
dashboards/views/               COMPLETE views, for the raw configuration editor.
                                sdr-meters.yaml is the first. Kept in the repo so
                                ha_audit.py can resolve its entity references - a
                                dashboard is the one place a broken entity is
                                completely silent: no log line, no unavailable
                                state, just an empty card.
