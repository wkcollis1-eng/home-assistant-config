...
SCHEMAS = {
    ...
    # cool_1f_hrs / cool_2f_hrs stub at 0 — cooling runtime sensors not yet
...
def cmd_append_utility_monthly(args):
    """Append one calendar-month row to utility_monthly.csv.

    Captures all utility billing fields and HVAC runtime totals available from
    HA at month-end.  cool_1f_hrs / cool_2f_hrs are written as 0 until cooling
    runtime sensors are implemented; the stubs keep the schema stable so
    downstream consumers don't break when cooling columns are later populated.
    """
    data = json.loads(args.data)
    filepath = REPORTS_DIR / "utility_monthly.csv"

    ensure_csv_exists(filepath, "utility_monthly")
    ...
    return append_row(filepath, data, "utility_monthly")
